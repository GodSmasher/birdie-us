...
const fmt = (iso?: string) => (iso ? new Date(iso).toLocaleDateString('de-DE') : '');
  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar
          title="Dateien & Wissen"
          subtitle={inFolder ? (searchParams.name ?? 'Ordner') : 'Google Drive · Wissensablage pro Bereich'}
        />
        <div className="flex-1 px-8 py-7 flex flex-col gap-6 max-w-[1100px]">
          {!drive.configured && (
            <Card className="p-8 text-center text-sm text-fg3 max-w-[560px] mx-auto mt-8">
              Google Workspace nicht verbunden — sobald verbunden, erscheint hier eure Drive-Struktur.
            </Card>
          )}
          ...
          {drive.scopeMissing && (
            <Card className="p-6 flex flex-col gap-3 max-w-[640px]">
              ...
              <h2 className="font-semibold text-base text-fg tracking-tightest">Drive-Zugriff noch nicht freigegeben</h2>
              <p className="text-[13px] text-fg2 leading-[20px]">
                Gmail & Kalender laufen schon. Für Drive fehlt nur der Scope <code className="text-accent">drive.readonly</code>:
              </p>
              <ol className="text-[13px] text-fg2 leading-[22px] list-decimal pl-5">
                <li>Google Cloud Console → „Google Drive API" aktivieren</li>
                <li>OAuth-Consent → Scope <code className="text-accent">.../auth/drive.readonly</code> hinzufügen</li>
                <li>Im OAuth-Playground den Refresh-Token mit dem Drive-Scope neu ziehen</li>
                <li>Neuen Token schicken → ich update die Env</li>
              </ol>
            </Card>
          )}
          ...
          {drive.configured && !drive.scopeMissing && !drive.error && (
            <>
              {inFolder && (
                <Link href="/dateien" className="text-[13px] text-accent font-medium w-fit">← Alle Bereiche</Link>
              )}

              {drive.folders.length > 0 && (
                <section className="flex flex-col gap-3">
                  <h2 className="font-semibold text-sm text-fg tracking-tightest">{inFolder ? 'Unterordner' : 'Bereiche'}</h2>
                    ...
                    {drive.folders.map((f) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-[11px] text-fg3">Ordner</span>
                        </div>
                      </Link>
                    ))}
                  </div>
              ...
              <Card className="overflow-hidden">
                <CardHeader title="Dateien" right={<Pill label="LIVE" tone="success" />} />
                {drive.files.length === 0 ? (
                  <div className="px-5 py-10 text-center text-sm text-fg3">Keine Dateien in diesem Ordner</div>
                ) : (
                  drive.files.map((f, i) => (
                    <a
                      key={f.id}
