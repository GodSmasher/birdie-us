  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-white">
      <header className="border-b border-white/10 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-white/50 mt-1">Documents for signing</p>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        {loading ? (
          <p className="text-white/40 text-center py-12 animate-pulse">Loading...</p>
        ) : projects.length === 0 ? (
          <div className="text-center py-16">
            <div className="text-4xl mb-4">&#x2713;</div>
            <p className="text-white/50 text-lg">No documents to sign</p>
            <p className="text-white/30 text-sm mt-2">All done! Keep this page open — new projects will appear automatically.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-6">
            <p className="text-white/40 text-sm">{projects.length} project{projects.length !== 1 ? 's' : ''} awaiting signature</p>
            ...
            {projects.map(p => (
              <div key={p.offerId} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/10">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-white/40">{p.address} &middot; {p.kwp} kW DC &middot; {p.netzbetreiber}</p>
                    </div>
                ...
                <div className="px-5 py-3 flex flex-col gap-2">
                  <p className="text-xs text-white/30 font-medium uppercase tracking-wide">Download documents:</p>
                  {p.documents.map(d => (
                    <a
                      key={d.form}
                      href={d.downloadUrl}
                ...
                <div className="px-5 py-4 border-t border-white/10 bg-white/[0.02]">
                  <p className="text-xs text-white/30 font-medium uppercase tracking-wide mb-2">Upload signed version:</p>
                  <label className={`flex items-center justify-center px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition
                    ...
                    <span className="text-sm text-white/50">
                      {uploading === p.offerId ? 'Uploading...' : 'Select or drop PDF here'}
                    </span>
                  </label>
                </div>
              </div>
